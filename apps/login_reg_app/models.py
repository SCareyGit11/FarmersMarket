...
class UserManager(models.Manager):
    def validateReg(self, request):
        ...
        user = self.create(name=request.POST['name'].lower(), uname=request.POST['uname'].lower(), hire_date=request.POST['hire_date'], pw_hash=pw_hash)
        return (True, user)
    ...
    def validate_inputs(self, request):
        i = datetime.strptime(request.POST['hire_date'], '%Y-%m-%d')
        errors = []
        ...
        if not request.POST['hire_date']:
            ...
            messages.error(request, "The date is not valid as it is in the future.", extra_tags='regis_hire_date')
